        ...
        if kk == EEG.nbchan:    #This is the trigger channel
            ...
            for ll in range(1,int(EEG.byte_8) + 1):
             ...
             for ll in range(1,int(EEG.byte_8) + 1):
                 ...
                 EEG.data[kk-1,start_data:end_data] = temp_bytes[0:int(sampl_freq_data)*3:3] + temp_bytes[1:int(sampl_freq_data)*3:3]*pow(2,8) + temp_bytes[2:int(sampl_freq_data)*3:3]*pow(2,16); 
                 EEG.data[kk-1,start_data:end_data] = EEG.data[kk-1,start_data:end_data] - pow(2,24)*(EEG.data[kk-1,start_data:end_data] >= pow(2,23));   #Complement 2 conversion
                 EEG.data[kk-1,start_data:end_data] = np.dot(EEG.data[kk-1,start_data:end_data],EEG.resolution[0,kk-1]);
                 
                 start_data  += int(sampl_freq_data);
                 end_data += int(sampl_freq_data);
                 bdf_file.seek((EEG.nbchan + 1)*256 + (kk - 1)*int(EEG.srate)*3 + ll*EEG.nbchan*int(EEG.srate)*3, 0);
